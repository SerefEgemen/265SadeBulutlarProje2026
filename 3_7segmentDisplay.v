...
modülün aldığı girdiler bu şekilde değerlendirilmektedir. 

karartmaSinyali değeri varsayılan olarak 1'dir. FSM modülü karartmaSinyali değerini 1 yapabilmektedir. Ana oyun FSM modülünde bu modülün tekrar çalışabilmesi için karartmaSinyali değeri 0'da yapılabilinecektir.

7 Segment displayde aynı anda sadece 1 hane çalıştırılabilmektedir fakat projede hanelerin aynı anda yakılması istenmiştir. Bu durumu çözmek adına şu yöntem izlenmiştir, Her bir hane aynı anda yakılamadığı için sıra sıra yakılmışlardır. Örnek olarak soldan sağa doğru haneleri
...
haneSayici değeri 1 oluğu zaman 2. hane yanar gibi. 

Outputlardaki an ve seg değerleri kullanılıp display çalıştırılır. an değeri kaçıncı hanenin seçileceğine karar verir. Seg ise hangi sayıyı gösterecek isek ona göre verdiğimiz değerdir. Bu iki output ters mantıkla çalışır.
Eğer 1. haneyi yakmak istiyorsam an değeri 4'b1000 değilde 4'b0111 şeklinde olmalıdır. 0 açık durumu, 1 ise kapalı durumu temsil eder. Aynı şey seg outputu içinde geçerlidir. Son register ise sayac registeridir. Sayac oyunun kurallarına göre hanelerin yanmaları arasındaki süreyi sayan yapıdır.
...
display üzerindeki 4 hanenin 4 üde yanıyordur birer sayı gösteriyorlardır. Tam bu anda bitisSinyali değeri 1 olur. Bu sinyal FSM'ye gider. FSM modülü bitiş sinyalini aldığı an LSFR den rastgele bir sayı seçer. LSFR modülü en fazla 5 saniyelik bir süre oluşturabilir. sayacın 9 saniye yani maksimum 900k adım almasının sebebi budur. 
Bitis sinyali gittikten sonra sayac saymaya devam eder çünkü Displayın kararmadan önce LSFR modülünün oluşturduğu x saniyelik süre kadar açık olması gerekmektedir. Sayac bu süreçte artar ve displayi açık tutar. FSM modülü LSFR nin ürettiği sürenin bitiminde bu modüle karartma sinyali yollar. Sinyal gelmez ise 
900_000_000. adıma gelindiğinde modül kendi kendini zaten karartacaktır. Fakat LSFR modülü her türlü bir değer üreteceği için bu durum gerçekleşmez. Sayac 900k ıncı adıma kadar displayı açık tutar. Bu süreç içerisinde eğer FSM karartmaSinyali yollar ise display
üzerinde ışıklar söner. Örnek olarak şunu düşünelim: 7seg modülü 4 saniye boyunca çalıştı ve en sonda 4 hanenin 4'üde yandı. 4. saniyede 7seg modülü bitiş sinyalini 1 yaptı. Bu sinyal FSM modülünü çalıştırdı. Bu FSM modülü sinyali aldığımda LSFR modülünden rastgele bir sayı aldı ve LSFR modülü örnek olarak 3.2 saniyelik bir süre oluşturdu.
Bu 3.2 saniye geçene kadar sayac halen her clock vuruşunda artmaya devam edecektir. Bitis sinyalinin 1 yapıldığı andan sonra 3.2 saniye sonra FSM modülü, 7seg modülüne karartmaSinyali yollayacaktır. Yani karartmaSinyali değeri 1 yapılacaktır. Bu sinyal geldiği zaman
sayac değeri henüz 900_000_000 olamadan sıfırlanır ayrıca display üzerinde yanan bütün ışıklar söndürülür. Bunun dışında FSM modülü içerisinde bu 7seg modülünden gelen sinyal posedge clock sinyalleriyle beraber kontrol edilmesi gerekli zira FSM modülüne yollanacak bitis sinyali
pulse şeklindedir. Yani bitis değeri 1 olduktan sonra bir clock döngüsü sonra 0 değerine geri gelir. Bu yüzden FSM modülünde bitis sinyali kontrolü yapılırken clock sinyalinin yükselen kolunda kontrol yapılmalı. Eğer bu şekilde kontrol edilirse 10 nanosaniyelik
bitis sinyalini yakalamak mümkün olacaktır.

Burdaki açıklamalar dışında altta açıklamalarda yapılmıştır.
...
always@(*) begin
    ...
    if(!resetSW15 && !kararmaSinyali) begin
        ...
        if(turNumarasi[0] == 1'b0) begin //Tur numarası çift sayı iken Displayde gösterilecek sayı sıralaması. Eğer sayının least significant biti 0 ise sayı tamamen çift sayıların toplamından oluşuyordur. Bu durumda sayı çift olacaktır çiftlik kontrolü yapılırken LSB ye bakılır.
            ...
            else if(sayac < 30'd899_999_999) begin // Display üzerinde 4 segmentin gösterileceği blok. İçerisinde 4 segmente erişen if blokları mevcut. Alttaki tur numarası tek olan içinki blokta neden 299_999_999 dan sonra bir anda 899_999_999 a atladığı yazılı. Bu sayılar temelde adım sayılarıdır. bu adım sayılarıyla 10 nanosaniyeyi çarpar isek kaçıncı daniyede olduğumuzu söylerler.
                hane = 2'd3;     
